...
object HotzoneAnalysis {
  ...
  def runHotZoneAnalysis(spark: SparkSession, pointPath: String, rectanglePath: String): DataFrame = {
    ...
    val joinDf = spark.sql("select rectangle._c0 as rectangle, point1._c5 as point1 from rectangle,point1 where ST_Contains(rectangle._c0,point1._c5)")
    joinDf.createOrReplaceTempView("joinResult")

    // YOU NEED TO CHANGE THIS PART

    val f_res = spark.sql("select rectangle, count(point1) from joinResult group by rectangle order by rectangle")
    return f_res.coalesce(1)
